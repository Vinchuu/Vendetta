...
app.get('/api/auth/discord/login', (req, res) => {
  ...
  if (!clientId || clientId === 'your_discord_client_id_here') {
    // Dev Mode Fallback redirect if Discord Client ID is not configured yet
    const token = `fyb_discord_dev_token_${mode}_${Date.now()}`;
    const frontendUrl = `http://localhost:8080/?token=${token}&mode=${mode}&username=DevUser_${mode}`;
...
app.get('/api/auth/discord/callback', async (req, res) => {
  ...
  if (!code || !clientId || !clientSecret || clientId === 'your_discord_client_id_here') {
    ...
    return res.redirect(`http://localhost:8080/?token=${token}&mode=${targetMode}&username=DiscordUser`);
  }
  ...
  try {
    ...
    if (!tokenResponse.ok || !tokenData.access_token) {
      ...
      return res.redirect(`http://localhost:8080/?auth_error=Failed+to+authenticate+with+Discord`);
    }
    ...
    // Check authorization by Discord User ID against .env whitelists
    const adminIds = (process.env.ADMIN_DISCORD_IDS || '').split(',').map(id => id.trim()).filter(Boolean);
    const memberIds = (process.env.MEMBER_DISCORD_IDS || '').split(',').map(id => id.trim()).filter(Boolean);
    ...
    if (targetMode === 'admin') {
      if (!isAdmin) {
        console.warn(`Unauthorized Leader login attempt: ${username} (${discordId})`);
        return res.redirect(`http://localhost:8080/?auth_error=Access+Denied:+Discord+ID+${discordId}+is+not+authorized+for+Leader+access.`);
      }
      finalMode = 'admin';
    } else {
      if (!isMember && !isAdmin) {
        console.warn(`Unauthorized Member login attempt: ${username} (${discordId})`);
        return res.redirect(`http://localhost:8080/?auth_error=Access+Denied:+Discord+ID+${discordId}+is+not+authorized+for+Member+access.`);
      }
      finalMode = 'gangmember';
    }

    const token = `fyb_discord_${discordId}_${Date.now()}`;
    return res.redirect(`http://localhost:8080/?token=${token}&mode=${finalMode}&username=${encodeURIComponent(username)}`);
  } catch (error) {
    console.error('Discord Auth Exception:', error);
    return res.redirect(`http://localhost:8080/?auth_error=Authentication+Failed`);
  }
});

// 3. Direct Login API (supports both Discord token verify and fallback)
...
app.put('/api/orders/:id', (req, res) => {
  ...
  if (updated) {
    // If order was marked as completed (and wasn't previously completed), auto-deposit to Syndicate Gang Fund & Money Moves
    if (updated.status === 'completed' && (!existingOrder || existingOrder.status !== 'completed')) {
      ...
      // 1. Record Income Transaction in Money Moves
      db.addTransaction({
      ...
      // 2. Deposit Funds into Gang Fund / Syndicate Bank
      ...
      // Broadcast financial updates live
      broadcastChange('transactions_updated', db.getTransactions());
...
app.post('/api/audit-logs', (req, res) => {
  const log = db.addAuditLog(req.body);
  broadcastChange('audit_logs_updated', db.getAuditLogs());
...
app.listen(PORT, () => {
  console.log(`🚀 Vendetta Backend Server running on http://localhost:${PORT}`);
});
